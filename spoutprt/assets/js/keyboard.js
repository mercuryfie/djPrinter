...
제작후기.
잠시 외박나와서 phpschool.com 의 Tip&Tech를 둘러보던 중 영타를 한글로 변환하는 글을 보게되었습니다.
행복한고니님께서 올리신 글인데 댓글 중에 자바 스크립트 버전이 있다고 하던데
저도 자바 스크립트 버전이 필요해서 찾아봤는데 찾기 어렵더군요 ^^;; 그래서 직접 만들게 되었습니다.
( 초성 순서 코드 * 21 * 28 ) + ( 중성 순서 코드 * 28 ) + 종성 순서 코드
위 방식대로만 하면 간단할 줄 알았는데 예를 들어 "&#49165;" 이란 문자와 "벡" 이란
문자가 있다면 &#49165;은 Qnpfr 이렇게 5개의 문자가 들어가고 벡은 qpr 이렇게 3개 문자면 되지요.
똑같은 초중종성으로 이루어졌어도 문자 수가 틀리고 또 모든 문자가 초성, 중성, 종성으로
이루어진게 아니라 "기차" 이런 단어들은 초성과 중성으로만 이루어져 있기 때문에
모든 경우의 수를 따지느라 좀 애먹었습니다. ^^;;
버그가 있을 수도 있지만 보이는 글씨들 다 쳐봤는데 잘되더군요.
시간이 많다면 capslock 눌림 체크, "." 문자 같은건 예외처리등의 부가 기능을
추가하고 싶지만 시간이 없는 관계로 다른 분에게 부탁드립니다. ^^;;

마지막으로 좋은 개발 경험을 하게 해주시고 좋은 정보를 주신 행복한고니님께 감사드립니다.
*/
